...
def main():
    ...
    with bayespy.data.DataSet(iris.drop('iris_class', axis=1), db_folder, logger) as dataset:
        ...
        model = bayespy.model.NetworkModel(network,
                                           dataset.subset(iris[(iris.iris_class == "Iris-versicolor") | (iris.iris_class == "Iris-virginica")].index.tolist()),
                                           logger)
        train_results = model.train()
        trained_network = train_results['network']

        variables = ['sepal_length','sepal_width','petal_length','petal_width']

        # query the trained model on all the data, including the Iris-setosa class
        model = bayespy.model.NetworkModel(trained_network, dataset, logger)
